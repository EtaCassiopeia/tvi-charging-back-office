  ...
  override def spec: ZSpec[_root_.zio.test.environment.TestEnvironment, Any] =
    suite("RefinedDataTypes")(
      testM("should be able to parse a valid energy consumed fee string") {
        for {
          res <- ZIO.fromEither(ConsumedEnergyFeePerKWH.fromString("0.25 EUR / kWh"))
        } yield assert(res)(equalTo(ConsumedEnergyFeePerKWH(Fee.from(0.25).toOption.get, Currency.getInstance("EUR"))))
      },
      testM("should be able to parse numbers without floating point as double") {
        for {
          res <- ZIO.fromEither(ConsumedEnergyFeePerKWH.fromString("1 EUR/kWh"))
        } yield assert(res)(equalTo(ConsumedEnergyFeePerKWH(Fee.from(1).toOption.get, Currency.getInstance("EUR"))))
      },
      testM("should raise an error in case the currency code is invalid") {
        for {
          res <- ZIO.fromEither(ConsumedEnergyFeePerKWH.fromString("0.25 AUR / kWh")).flip
      ...
      testM("should raise the parse failure error for an invalid fee expression") {
        for {
          res <- ZIO.fromEither(ConsumedEnergyFeePerKWH.fromString("0.a5 EUR / kWh")).flip
        } yield assert(res)(
          equalTo(
